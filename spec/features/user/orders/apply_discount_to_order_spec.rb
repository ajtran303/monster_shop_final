...
RSpec.describe "Apply Discount to Order Spec" do
  before :each do
    ...
    @discounted_subtotal1 = "#{number_to_currency(@discount1.calculate_discount(@item1.price * 5))}"
    @discount_notice1 = "Discount applied: #{@discount1.percentage_discount}% off for #{pluralize(@discount1.bulk_quantity, @order_item1.item.name)}!"
    ...
    describe "When I have an order with an item eligible for a bulk discount" do
      it "I see the discounted price on my order show page" do
        ...
        within "#order-item-#{@order_item1.id}" do
          expect(page).to have_content("Subtotal: #{@discounted_subtotal1}")
