...
RSpec.describe "Apply Discount in Cart Spec" do
  before :each do
    ...
    @discount_notice1 = "Discount applied: #{@discount1.percentage_discount}% off for #{pluralize(@discount1.bulk_quantity, @item1.name)}!"
    @discounted_subtotal1 = "Subtotal: #{number_to_currency(@discount1.calculate_discount(@item1.price * 5))}"

    @discount_notice2 = "Discount applied: #{@discount2.percentage_discount}% off for #{pluralize(@discount2.bulk_quantity, @item1.name)}!"
    @discounted_subtotal2 = "Subtotal: #{number_to_currency(@discount2.calculate_discount(@item1.price * 10))}"
  ...
  describe "As a visitor" do
    describe "When I add a bulk quantity of items to my cart" do
      it "The price is adjusted with a percentage discount" do
        within "#item-#{@item1.id}" do
          ...
          expect(page).to_not have_content(@discount_notice1)
        end
        ...
        within "#item-#{@item1.id}" do
          expect(page).to have_content(@discounted_subtotal1)
          expect(page).to have_content(@discount_notice1)
        end
        ...
        # cart total updates with correct price
    ...
    describe "When I adjust the quantity of items in my cart" do
      it "The discount is automatically added when I meet the bulk quantity" do
        within "#item-#{@item1.id}" do
          expect(page).to have_content("Subtotal: #{number_to_currency(@item1.price * 4)}")
          expect(page).to_not have_content(@discount_notice1)
      ...
      it "The discount is automatically removed when I no longer meet the bulk quantity" do
        visit item_path(@item1)
        click_button "Add to Cart"
        visit "/cart"
      ...
      it "The discount is automatically adjusted when I meet a higher bulk quantity" do
        within "#item-#{@item1.id}" do
          click_button("More of This!")
        ...
        within "#item-#{@item1.id}" do
          ...
          expect(page).to have_content(@discounted_subtotal2)
          expect(page).to have_content(@discount_notice2)

          expect(page).to_not have_content(@discounted_subtotal1)
          expect(page).to_not have_content(@discount_notice1)
        end
      end
      ...
      it "The discount is automatically adjusted when I meet a lower bulk quantity" do
        ...
        within "#item-#{@item1.id}" do
          ...
          expect(page).to_not have_content(@discounted_subtotal2)
          expect(page).to_not have_content(@discount_notice2)
        end
      end
    end
  end
